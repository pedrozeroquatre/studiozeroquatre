...
async function lireTout(sb) {
  const [
    ...
    sb.from('portail_mes_livraisons').select(COLONNES.livraisons).order('date_livraison', { ascending: false }),
...
export async function ouvrirPaiement({ lignes, date, note, etablissement }) {
  const sb = getEspaceSupabase()
  if (!sb) throw new Error('Espace non configuré')
  ...
  const res = await fetch('/api/espace/checkout', {
    ...
    body: JSON.stringify({ jeton: session.access_token, lignes, date, note, etablissement }),
  })
